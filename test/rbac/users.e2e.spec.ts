...
describe('RBAC - Users (e2e)', () => {
	...
	describe('Viewer role', () => {
		...
		it('should allow GET own user by id', async () => {
			...
			const response = await request.get(usersRoutes.getById(viewerUserId)).set(viewerHeaders);
		...
		it('should deny DELETE other users', async () => {
			...
			const response = await request.delete(usersRoutes.delete(editorUserId)).set(viewerHeaders);
	...
	describe('Editor role', () => {
		...
		it('should deny DELETE other users', async () => {
			...
			const response = await request.delete(usersRoutes.delete(viewerUserId)).set(editorHeaders);
	...
	describe('Admin role', () => {
		it('should allow full CRUD on users', async () => {
			...
			// GET all
			const getAllResponse = await request.get(usersRoutes.getAll).set(adminHeaders);

			expect(getAllResponse.status).toBe(StatusCodes.OK);

			// GET by id
			const getByIdResponse = await request.get(usersRoutes.getById(userId)).set(adminHeaders);
			...
			// DELETE
			const deleteResponse = await request.delete(usersRoutes.delete(userId)).set(adminHeaders);
