...
describe('Comments (e2e)', () => {
	...
	afterAll(async () => {
		...
		if (testArticleId) {
			await unauthorizedRequest.delete(articlesRoutes.delete(testArticleId)).set(commonHeaders);
		}
	...
	describe('POST', () => {
		...
		it('should respond with BAD_REQUEST in case of invalid required data', async () => {
			const responses = await Promise.all([
				unauthorizedRequest.post(commentsRoutes.create).set(commonHeaders).send({}),
				unauthorizedRequest
					.post(commentsRoutes.create)
					.set(commonHeaders)
